package duyndph34554.fpoly.lab_6.ui.screens
...
import duyndph34554.fpoly.lab_6.model.entities.Seat
import duyndph34554.fpoly.lab_6.model.entities.SeatStatus
import duyndph34554.fpoly.lab_6.ui.compoments.SeatComposable
import kotlin.random.Random

@Composable
fun CinemaSeatBookingScreen(seats: List<Seat>, totalSeatsPerRow: Int){
    val textModifier = Modifier.padding(end = 14.dp, start = 4.dp)
    ...
    Column (
        ...
        Row (
            ...
            val exampleEmptySeat = remember {
                Seat('X', 1, SeatStatus.EMPTY)
            }

            val exampleSelectedSeat = remember {
                Seat('Y', 1, SeatStatus.SELECTED)
            }

            val exampleBookedSeat = remember {
                Seat('Z', 1, SeatStatus.BOOKED)
            }

            SeatComposable(seat = exampleEmptySeat, false)
            ...
            SeatComposable(seat = exampleSelectedSeat, false)
            ...
            SeatComposable(seat = exampleBookedSeat, false)
...
fun createTheaterSeating(
    ...
    for (rowIndex in 0 until totalRows) {
        for (seatIndex in 1..totalSeatsPerRow) {
            val adjustedRowIndex = if (rowIndex >= aislePositionInRow)
